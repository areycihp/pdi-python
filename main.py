...
class Imagen(object):
    ...
    def preprocesamiento(self):
        ...
        ret, im_binaria = cv.threshold(
            im_gris, 0, 255, cv.THRESH_BINARY | cv.THRESH_OTSU)
        ...
        apertura = cv.morphologyEx(
            im_binaria, cv.MORPH_OPEN, kernel=kernel, iterations=2)
        ...
        ret, im_binaria2 = cv.threshold(
            im_combinada, 0, 255, cv.THRESH_BINARY | cv.THRESH_OTSU)
        ...
        Nueva = cv.imwrite("Images/NewImages/"+self.nombre +
                           "."+self.formato, im_binaria2)
        ...
        ret, superficie = cv.threshold(
            distancia_salida, distancia_salida.max() * 0.3, 255, cv.THRESH_BINARY)

        # Marcadores de superficie a 8 bits - Técnicas y funciones de python (14)
        superficie_8 = np.uint8(superficie)
        superficie_desconocida = cv.subtract(
            dilatacion, superficie_8)  # dilatacion
        # Etiquetado
